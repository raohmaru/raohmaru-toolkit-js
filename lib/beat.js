...
export class Beat {
    ...
    frame(currentTime) {
        ...
        this._currentTime = currentTime - this._pausedTime;
        // Calculate elapsed time since last loop
        let delta = this._currentTime - this._previousDeltaTime;
        // If enough time has elapsed, draw the next frame
        if (delta >= this._fpsInterval) {
            // Adjust next execution time in case this frame took longer to execute
            this._previousDeltaTime = this._currentTime - (delta % this._fpsInterval);
            this._cb(this._currentTime, this._currentTime - this._previousTime);
